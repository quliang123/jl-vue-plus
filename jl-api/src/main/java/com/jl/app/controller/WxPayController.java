...
import com.jl.app.config.pay.WxPayConfiguration;
import com.jl.app.config.pay.WxPayProperties;
import com.jl.app.utils.PayUtil;
import com.jl.app.utils.WxPayUtil;
...
@Api(value = "微信支付小程序控制器", tags = {"微信支付管理管理"})
public class WxPayController {
    ...
    public AjaxResult createOrder(WxPayUnifiedOrderRequest orderRequest, TranOrder tOrder) throws WxPayException {
        String logPrefix = "【募捐统一下单】";

        String outTradeNo = PayUtil.getTradeNo();

        //设置支付参数
        orderRequest.setNotifyUrl(wxPayProperties.getNotifyUrl());
        orderRequest.setTradeType(WxPayConstants.TradeType.JSAPI);
        orderRequest.setOutTradeNo(outTradeNo);
        orderRequest.setBody(orderRequest.getBody());
        orderRequest.setTotalFee(Integer.valueOf(orderRequest.getTotalFee()));
        orderRequest.setOpenid(orderRequest.getOpenid());
        orderRequest.setSpbillCreateIp(ServletUtils.getRequest().getLocalAddr());
        ...
        orderRequest.setTimeExpire(sdf.format(expireTime));

        //保存订单流水信息
        User user = userService.getByUserId(tOrder.getUserId());
        logger.info("{}  保存数据为 {}", logPrefix, tOrder.toString());
        if (Objects.equals(user, null)) {
            logger.error("{}用户", tOrder.getUserId());
            return AjaxResult.error("募捐失败,当前用户已失效，麻烦重新登录");
        }

        //创建订单对象
        PayOrder payOrder = new PayOrder();
        payOrder.setOpenId(orderRequest.getOpenid());
        payOrder.setUserId(Integer.valueOf(String.valueOf(user.getUserId())));
        payOrder.setOrderNo(outTradeNo);
        payOrder.setAmount(Long.valueOf(tOrder.getTotalFee()));
        payOrder.setStatus(1);
        payOrder.setClientIp(ServletUtils.getRequest().getLocalAddr());
        payOrder.setBody(tOrder.getBody());
        payOrder.setNotifyUrl(orderRequest.getNotifyUrl());
        payOrder.setExpireTime(expireTime);
        payOrder.setPaySuccTime(DateUtils.getNowDate());
        payOrder.setChannelUser(user.getOpenId());
        payOrder.setCreateBy(user.getUsername());
        payOrder.setCreateTime(now);

        //保存订单流水
        logger.info("{} 开始创建订单 保存数据为 {}", logPrefix, payOrder.toString());

        //生成订单         0-订单生成
        AjaxResult payResult = orderService.createPayOrder(payOrder);
        logger.info("{} 创建支付订单,结果为{}", "募捐支付", logPrefix, payResult.toString());
        if (!Objects.equals(payResult.getData(), 1)) {
            return AjaxResult.error("保存支付订单失败");
        }

        //保存捐款信息
        logger.info("{} 开始保存募捐信息 保存数据为 {}", logPrefix, tOrder.toString());
        boolean result = activityService.addDoneInfo(tOrder);
        logger.info("{} 开始保存募捐信息,结果为 {}", logPrefix, result);

        if (!result) {
            return AjaxResult.error("募捐相关信息保存失败，请联系管理员");
        }

        logger.info("{} 支付参数 {}", logPrefix, orderRequest.toString());

        //暂时不进行支付
        Object data = this.wxService.createOrder(orderRequest);
    ...
    public AjaxResult parseOrderNotifyResult(@RequestBody String xmlData) throws WxPayException {
        ...
        String attach = notifyResult.getAttach();//接收备注里存的账单id 或者预定id
        String totalFee = BaseWxPayResult.fenToYuan(notifyResult.getTotalFee());

        if (!"SUCCESS".equalsIgnoreCase(notifyResult.getResultCode())) {
            logger.error("微信小程序支付回调异常");
            return AjaxResult.error("微信小程序支付回调异常");
         }

        AjaxResult ajaxResult = iPayOrderService.selectPayOrderByOrderId(Long.valueOf(orderId));
        Object order = ajaxResult.getData();
        if (Objects.equals(order, null)) {
            logger.error("{}订单不存在！",orderId);
            return AjaxResult.error("订单不存在,请尽快管理员尽快处理");
        }

        PayOrder payOrder = JSONObject.parseObject(order.toString(), PayOrder.class);
        //将订单状态改成支付成功
        payOrder.setStatus(2);

        AjaxResult result = iPayOrderService.updatePayOrder(payOrder);
        Object data = result.getData();
        if (Objects.equals(data, 0)) {
            logger.error("{}订单改变状态失败！",orderId);
            return AjaxResult.error("订单改变状态失败,请尽快管理员尽快处理");
        }
